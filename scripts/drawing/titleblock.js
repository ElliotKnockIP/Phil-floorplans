// Reverted titleblock.js with simplified utils compatibility
import { closeSidebar, setupColorPicker, showDrawingPopup, hideDrawingPopup, setCrosshairCursor, setDefaultCursor } from "./drawing-utils.js";
...
export function setupTitleBlockTool(fabricCanvas) {
  ...
  const config = {
    ...
    fontFamily: "Arial",
    cellPadding: 10,
    maxChars: 37, // Increased from 38 to allow more text per line
  };
          ...
          if (word.length > config.maxChars) {
            ...
            for (let i = 0; i < word.length; i += config.maxChars) {
              ...
              if (i === 0 && word.length > config.maxChars) {
                ...
                currentLine = chunk; // Last chunk becomes start of next line
              } else {
                lines.push(chunk);
              }
            }
