// save/camera-device-save.js - MODIFIED VERSION
...
class CameraDeviceSerializer {
    ...
    try {
      ...
      if (serializedData.cameraDevices?.length) {
        for (let i = 0; i < serializedData.cameraDevices.length; i++) {
          try {
            await this.loadCameraDevice(serializedData.cameraDevices[i]);
  ...
  async loadCameraDevice(deviceData) {
        ...
        fabric.Image.fromURL(
          ...
          (img) => {
            try {
              ...
              // NEW: Use defaultDeviceIconSize for scaling
              ...
              const circle = new fabric.Circle({
                ...
                originX: "center",
                originY: "center",
                scaleX: savedCircleData?.scaleX || scaleFactor, // NEW: Apply scaleFactor to circle
                scaleY: savedCircleData?.scaleY || scaleFactor, // NEW: Apply scaleFactor to circle
              ...
              const group = new fabric.Group([circle, img], {
                ...
                ...deviceData.visual,
                scaleFactor: deviceData.scaleFactor || scaleFactor, // NEW: Use scaleFactor
              });
