...
export function addCameraCoverage(fabricCanvas, cameraIcon) {
  ...
  cameraIcon.coverageConfig.isInitialized = true; // Mark as initialized
  ...
  function createCoveragePoints(startAngle, endAngle, camCenterX, camCenterY) {
    ...
    // For full circle, create a perfect circle instead of polygon points
    if (isFullCircle) {
      // Return points for a circle (this will be handled differently in updateCoverage)
      const numPoints = 100; // High number of points for smooth circle
      const points = [];
    ...
    // For partial coverage, use the existing logic
    const points = [];
    const camCenter = { x: camCenterX, y: camCenterY };
    points.push(camCenter);

    let adjustedEndAngle = endAngle;
    if (endAngle <= startAngle) adjustedEndAngle += 360;

    const numPoints = Math.max(10, Math.ceil(angularSpan / 3));
    const angleStep = angularSpan / numPoints;
