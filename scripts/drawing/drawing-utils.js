...
export function setDrawingModeCursors(fabricCanvas, enable, excludeCameraImages = true) {
  ...
  fabricCanvas.getObjects().forEach((obj) => {
    // Skip objects with isResizeIcon property to preserve their hoverCursor
    if (obj.isResizeIcon) {
      obj.set({ selectable: !enable });
      return;
    }

    let cursor = enable ? "crosshair" : "default";

    // Set selectable property based on drawing mode
    obj.set({ selectable: !enable });

    if (!enable) {
      // Keep background images non-selectable
      if (obj.isBackground || (obj.type === "image" && obj.selectable === false)) {
        obj.set({ selectable: false, hoverCursor: "default" });
        return; // Skip further processing for background images
      }
      ...
      if (obj.type === "line" || obj.type === "arrow" || obj.type === "rect" || obj.type === "circle" || obj.type === "polygon") {
        ...
        // Handle fabric.Text objects (like device labels)
        cursor = "default";
        if (obj.isDeviceLabel) {
          obj.set({ selectable: false });
        } else {
          obj.set({ selectable: true });
        }
      } else if (obj.type === "group") {
        if (obj.deviceType && obj.deviceType.includes("camera")) {
          cursor = "move";
        } else {
          cursor = "default";
        }
