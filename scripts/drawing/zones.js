...
export function setupZoneTool(fabricCanvas) {
  ...
  const createZoneObjects = (polygonPoints, zoneName, zoneNotes, height, area, volume) => {
    ...
    text.setControlsVisibility({ mt: false, mb: false, ml: false, mr: false, bl: false, br: false, tl: false, tr: false, mtr: true });
  ...
  const addZoneEventHandlers = (finalPolygon, text) => {
    ...
    finalPolygon.on("mousedown", (e) => {
      ...
      e.e.preventDefault();
      e.e.stopPropagation();
      fabricCanvas.setActiveObject(devicesUnderneath.length > 0 ? devicesUnderneath[0] : text);
      fabricCanvas.requestRenderAll();
    });
  };
  ...
  const resetDrawingState = () => {
    currentZonePoints = [];
    [currentPolygon, previewLine, startPointCircle].forEach((obj) => obj && fabricCanvas.remove(obj));
    currentPolygon = previewLine = startPointCircle = null;
    hideDrawingPopup();
  ...
  // Handle keyboard events for zone creation and deletion
  document.addEventListener("keydown", (e) => {
    if (isCreatingZone) {
      if (e.key === "Enter" && currentZonePoints.length > 2) return completeZone(), true;
      if (e.key === "Escape") return resetDrawingState();
    }
    ...
    if (e.key === "Delete" || e.key === "Backspace") {
      ...
      if (activeObject && ((activeObject.type === "polygon" && activeObject.class === "zone-polygon") || (activeObject.type === "i-text" && activeObject.class === "zone-text"))) {
        if (activeObject.type === "i-text" && activeObject.isEditing) return e.preventDefault(), e.stopPropagation();
