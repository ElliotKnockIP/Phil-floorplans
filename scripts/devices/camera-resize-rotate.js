...
// Function to create or update the coverage area for a camera icon
export function createOrUpdateCoverageArea(fabricCanvas, cameraIcon) {
  ...
  function ensureZOrder() {
    ...
    // Always ensure proper z-order: coverage < camera < resize icons < text
    if (isSelectedCamera) {
    ...
    // Always bring text to front, regardless of selection state
  ...
  if (!leftResizeIcon || !rightResizeIcon || !rotateResizeIcon) {
    ...
    iconUrls.forEach(({ url, cursor, prop }) => {
      fabric.Image.fromURL(url, (icon) => {
        ...
        loadedCount++;
        if (loadedCount === 3) {
          // Handle mouse movement for resizing/rotating
