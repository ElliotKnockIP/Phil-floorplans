...
export function createOrUpdateCoverageArea(fabricCanvas, cameraIcon) {
  ...
  function updateCoverage() {
    ...
    const baseProps = {
      ...
      fill: fillColor,
      isCoverage: true,
      // CRITICAL: Ensure coverage area doesn't interfere with resize icons
      evented: false,
      selectable: false,
    };
    ...
    // **CRITICAL FIX**: Insert coverage area at a specific position to ensure proper layering
    ...
    if (leftResizeIcon) {
      leftResizeIcon
        .set({
          ...
          // CRITICAL: Ensure high z-index behavior
          evented: true,
          selectable: false,
        })
        .setCoords();

      // **CRITICAL**: Always bring resize icons to absolute front
      if (shouldShowIcons) {
    ...
    // **FINAL STEP**: Ensure proper z-order after everything is updated
    ensureProperZOrder();
    fabricCanvas.requestRenderAll();
  }

  // NEW FUNCTION: Add this to camera-resize-rotate.js
  function ensureProperZOrder() {
    ...
    // **MOST CRITICAL**: Bring all resize icons to absolute front
  ...
  // Don't force visibility - respect the saved state
  ...
  function stopResizing() {
    ...
    [leftResizeIcon, rightResizeIcon, rotateResizeIcon].forEach((icon) => {
      if (icon) icon.set({ visible: shouldShowResizeIcons });
    });
  ...
  if (!leftResizeIcon || !rightResizeIcon || !rotateResizeIcon) {
    const iconConfig = {
      ...
      opacity: layers.devices.opacity,
      isResizeIcon: true,
      // CRITICAL: Higher priority for mouse events
    ...
    iconUrls.forEach(({ url, cursor, prop }) => {
      fabric.Image.fromURL(url, (icon) => {
        ...
        // **CRITICAL FIX**: Enhanced mousedown handler that prevents event bubbling

        // Enhanced mouse down handler that prevents event bubbling - UPDATED VERSION
        icon.on("mousedown", (opt) => {
          ...
          // **NEW**: Also temporarily disable other drawing objects during resize
          ...
          // Enhanced stop resizing function
          const enhancedStopResizing = (e) => {
            // **CRITICAL**: Prevent any mouse up event from bubbling
            if (e && e.e) {
              e.e.preventDefault();
              e.e.stopPropagation();
            ...
            // **CRITICAL**: Re-enable wall interactions AFTER a delay
            setTimeout(() => {
              wallObjects.forEach((wall) => {
                wall.set({
        ...
        // **NEW**: Add mouseover/mouseout handlers to ensure icon stays on top
        icon.on("mouseover", () => {
          icon.bringToFront();
          fabricCanvas.renderAll();
        });
        ...
        loadedCount++;
        if (loadedCount === 3) {
          // **ENHANCED**: Mouse movement handler with better event handling
