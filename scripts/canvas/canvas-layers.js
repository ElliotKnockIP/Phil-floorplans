    ...
    for (const layer in layers) {
      layers[layer].objects.forEach((obj) => {
        if (obj && typeof obj.set === "function") {
          ...
          // Update visibility of resize icons for CCTV devices
          if (layer === "cctv" && (obj.leftResizeIcon || obj.rightResizeIcon || obj.rotateResizeIcon)) {
            if (obj.leftResizeIcon) obj.leftResizeIcon.set({ visible: isVisible && obj.coverageConfig?.visible });
            if (obj.rightResizeIcon) obj.rightResizeIcon.set({ visible: isVisible && obj.coverageConfig?.visible });
            if (obj.rotateResizeIcon) obj.rotateResizeIcon.set({ visible: isVisible && obj.coverageConfig?.visible });
          }
        }
      });
    }
