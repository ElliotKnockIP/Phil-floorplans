...
import { closeSubSidebar, toggleDrawingModePopup, setDrawingModeCursors, registerDrawingMode, resetDrawingState } from "./drawing-utils.js";
...
export function initCreateZones(fabricCanvas) {
  ...
  // Layer management - simplified but comprehensive
  const maintainLayerOrder = () => {
    ...
    // Zones above background, below devices
    ...
    // Devices and drawing objects to front
    objects.forEach((obj) => {
      if ((obj.type === "group" && obj.deviceType) || ["line", "circle", "rect"].includes(obj.type) || (obj.type === "i-text" && !obj.class)) {
        obj.bringToFront();
        if (obj.textObject && objects.includes(obj.textObject)) {
          obj.textObject.bringToFront();
        }
      }
    });

    // Zone text above polygons but below devices
    window.zones.forEach((zone) => {
  ...
  // Create zone objects with shared properties and event handlers
  ...
  const addZoneEventHandlers = (finalPolygon, text) => {
    ...
    // Movement handlers
    const handleMovement = (movingObj, staticObj) => {
      movingObj.on("moving", () => {
        if (!finalPolygon || !fabricCanvas.getObjects().includes(finalPolygon)) return;
        if (staticObj && fabricCanvas.getObjects().includes(staticObj)) {
          const newCenter = finalPolygon.getCenterPoint();
          staticObj.set({
            left: newCenter.x + (staticObj.offsetX || 0),
            top: newCenter.y + (staticObj.offsetY || 0),
          });
          staticObj.setCoords();
        }
        fabricCanvas.requestRenderAll();
      ...
      movingObj.on("moved", () => setTimeout(maintainLayerOrder, 10));
    };

    handleMovement(finalPolygon, text);
  ...
  const resetDrawing = () => {
    currentZonePoints = [];
    [currentPolygon, previewLine].forEach((obj) => {
      if (obj) {
        fabricCanvas.remove(obj);
      }
    });
